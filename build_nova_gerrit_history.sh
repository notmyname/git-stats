...
QUERY="project:openstack/nova branch:master limit:$PAGE_SIZE --comments --all-reviewers"
...
QUERY="project:openstack/nova branch:master status:open NOT label:Workflow\<=-1 limit:$PAGE_SIZE --comments --all-reviewers"
ssh -p29418 notmyname@review.openstack.org gerrit query --format JSON $QUERY >$PATCHES
MORE=`tail -20 $PATCHES | sed -e 's/[{}]/''/g' | awk -v k="text" '{n=split($0,a,","); for (i=1; i<=n; i++) print a[i]}' | grep moreChanges | cut -d: -f2 | cut -d: -f2 | sed 's/"//g' | tail -1`
I=1
while [[ $MORE == "true" ]]; do
